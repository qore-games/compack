...
mkdir -p compack.AppDir/assets

# Copy the built binary
cp zig-out/bin/compack compack.AppDir/usr/bin/
chmod +x compack.AppDir/usr/bin/compack

# Copy the compack CLI binary into the AppImage (app expects it at assets/compack)
cp assets/compack compack.AppDir/assets/compack
chmod +x compack.AppDir/assets/compack
...
# Create AppRun (cd to AppDir so relative path assets/compack resolves)
